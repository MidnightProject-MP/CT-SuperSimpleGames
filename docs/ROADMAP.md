...
| K2 Device passes | Physical touch-device checks (gesture protection feel, drag ease where relevant, frame geometry) and airplane-mode exercise across launcher → games → Home | **Partially closed 2026-09-03:** airplane-mode offline shell **PASSED** on physical device (owner provenance: `v51` online → close → airplane → relaunch → launcher + all games offline — corroborates 194 + rendered probe). Remaining: gesture feel / drag ease on real hardware still unobserved; does not block L3. |
...
*Owner direction (2026-08-24): replace fixed caregiver-selected levels with per-game adaptive progression — continuous, hidden, game-specific, slow, reversible, never experienced as levels, scores, promotion, or failure. Each world defines its minimum and maximum experience plus the interaction signals that actually matter to it; every fresh entry starts at minimum, while historical fluency only controls how quickly current-session evidence can expand the experience. Maximum means purposeful spatial richness as well as cognitive richness: unused space should be intentional, not an artifact of minimum-first design. Companion rule: measure the interaction the game cares about — do not invent generic metrics because they are countable. No shared adaptive engine until game-specific implementations prove common shape.*
...
| L2 Memory adaptive prototype | First implementation: versioned local fluency record (`supersimplegames.memory.adaptive`, gameplay state only); envelope = sparse 2×2 minimum → purposeful 3×2 maximum · preview 1.9→1.4s · mismatch window 1.05→0.85s · arrangement variety; exponential average with hysteresis tiers; ~25–30% of rounds serve one step simpler than estimate (comfort variation); at most one dimension moves per completed round; every fresh entry starts at minimum and history controls ramp speed; grown-ups gains "Reset playful growth"; deterministic tests + rendered probes | **Refined 2026-08-24:** session frontier starts at minimum even with a rich persisted record; current evidence gates one-notch enrichment; 14 dedicated adaptive tests plus rendered launch/ramp probe. **Gate remains re-armed:** hands-on decides whether the adaptation feels like meeting the child where they are |
| L3 Color Splash adaptive board mix | Second application: identity count (3–4) and family-tier mix adapt within envelope from completed-board evidence; extract a tiny shared adaptive-state helper **only if** Memory's record shape proves identical | Blocked on L2 gate only (Memory hands-on). Offline P0 gate **CLOSED** 2026-09-03 — physical-device airplane validation PASSED and corroborates `v51` fix; do not reopen offline unless new device evidence contradicts. |
| L4 Fixed-level retirement | Remove the Default/Gentle/Rich control, `level` settings field, and every consumer (Bloom unified 24-object garden; Color Splash fixed 4 identities; Numbers visible row stays 1·2·3); delete retired Stack/Tones sources, pages, styles, and tests; sanitizer drops stored level harmlessly; grown-ups page keeps creations/reset controls and gains the growth reset | **Closed** 2026-08-24: level panel and field removed with graceful fallback for stored settings; Bloom/Color Splash/Numbers constants unified; retired worlds fully deleted from the tree (git retains history); 179 deterministic checks green after the L2 session-ramp refinement |
...
- **2026-09-03 offline shell (hard constraint, P0): FAILED then FIXED and DEVICE-VALIDATED:** owner report `v50` PWA could not open games in airplane mode despite 179/179 → `v51` cache-first + `allSettled` per-entry precache + navigation-variant fallback → 194 checks + rendered Edge offline probe (62 cached, all 6 worlds + caregiver cold-load offline, core interactions offline) → **2026-09-03 physical-device PASSED** (owner provenance: online → close → airplane → relaunch from icon → launcher + all games offline). Offline gate **CLOSED**; L3 no longer blocked by offline. See `docs/EVIDENCE.md` 2026-09-03 and `STATE.md`.
...
- **Epic L acceptance was given by the owner; L2+L4 are implemented and verified.** Remaining: hands-on signal on the adapting Memory, then L3 (now gated **only** on L2); observation for L5.
- E1 identity directions remains deferred. K proceeds opportunistically for remaining gesture-feel and observation work.
