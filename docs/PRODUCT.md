...
Adult concerns live behind one gated adult surface, kept away from child play. The launcher footer holds a small, muted "For grown-ups" control that opens only after a 1500 ms press-and-hold with visible progress; quick taps do nothing (keyboard users activate directly for accessibility). The caregiver page owns: the sound default (shared with the child-facing in-game toggles), session length (off/10/15/20/30 minutes — stored for the wind-down ritual), which worlds appear on the launcher (the last visible world cannot be hidden), and a confirmed reset that clears all local settings and saved creations. Every change persists immediately; nothing is transmitted. Settings are validated field-by-field and fall back to defaults when stored data is missing, malformed, or storage is restricted. The child-facing interface gained only the muted footer gate.

### Adaptive progression

**Principle (owner-set 2026-08-24):** each world defines a bounded minimum and maximum experience and its own meaningful signals of interaction fluency. Every fresh entry begins at minimum; historical fluency is only a prior for how quickly current-session evidence may enrich the experience. The world gradually and reversibly adapts within that envelope based on recent play, without visible scores, failure states, levels, or abrupt difficulty jumps. Adaptation enriches or simplifies the experience rather than rewarding or punishing the child. Minimum may be intentionally sparse and huge-targeted; maximum should make fuller purposeful use of the play surface without filling space decoratively. Companion rule: measure the interaction the game actually cares about — never invent generic performance metrics merely because they are easy to count.

How it behaves: evidence updates once per natural round boundary; an exponential average with hysteresis tiers provides inertia (toddlers are inconsistent; one round proves little); at most one meaningful dimension moves per step; roughly a quarter of served experiences come one step simpler than current estimate, preserving variation and confidence even for fluent players. Internal fluency records are minimal local gameplay state — versioned, resettable from the grown-ups surface, never shown to the child, and framed as "how much of this game's experience seems useful right now," nothing diagnostic.
...
## Numbers: experimental quantity world

Numbers is a deliberately thin experiment answering one question from owner direction: do big numerals plus immediate visual consequence make quantity playful? Three oversized tappable number bubbles — 1, 2, 3 — sit beneath an open meadow scene; tapping one makes **that many** friendly characters burst into the meadow with staggered bouncy arrivals, each arrival playing its own rising tone so the group counts itself aloud, while a giant ghost of the tapped numeral flashes across the sky. Every arrived friend is individually touchable and hops with its own voice. Tapping another number waves the current group goodbye and brings the new one in; tapping the same number rearranges it freshly. The number set remains deliberately fixed at 1–3 while its adaptive envelope is still gated by evidence. Word counts accompany events for assistive tech, but the numerals themselves are the interface — no reading, no quiz, no request, no wrong answer, and free repetition is the whole game. It does **not** claim to teach arithmetic, and it excludes counting tasks, autoplay, and escalation pressure.
